...
Phase 2.1: Core Neural System with Memory + Voice + TTS Options + Vision
"""
...
from agent.vision_integration import add_vision_to_agent  # VISION: Added import
...
def main():
    ...
    print("Brain + Memory + Semantic Search + Voice + TTS Options")
    print("=" * 60)
    print()
    ...
    try:
        ...
        voice = initialize_voice_interface(
            ...
            speech_rate=1.3,  # Normal speed
            porcupine_access_key='iZGN+6LApfHvF+snSpDyswiM6jREUEQV4V1mg7Gp0go64ReXQ9Mcyg=='
        )
        print("✓ Voice interface ready")
        print(f"✓ Default TTS: {voice.get_tts_mode().upper()}")
        
        print("🤖 Initializing agent with semantic memory...")
        AGENT_SOURCE_PATH = r"C:\Users\aalldredge-da\Desktop\grok_phase2\filesPhase2\grok_agent_phase2\grok_agent\agent"
        agent = initialize_agent(grok, db, embedder, memory, chore_db, plc_comm, AGENT_NAME, AGENT_SOURCE_PATH)
        
        # Attach introspection parser so text commands work
        ...
        print(f"✓ {AGENT_NAME} is ready with introspection!")
        print()
        
        # VISION: Add vision capabilities to agent
        print("👁️  Adding vision capabilities...")
        agent = add_vision_to_agent(agent, device="auto")
        ...
        print("Voice + Text Interface + TTS Options")
        print()
        print("Commands:")
        ...
        print("  'piper' - Switch to Piper TTS (offline)")
        print("  'gtts' - Switch to gTTS (online)")
        print("  'tts' - Show current TTS mode")
        print("  'exit' - quit")
        print("  'new' - new conversation")
        print("  'history' - recent conversations")
        print("  'memory' - memory statistics")
        print("  'see' - Capture and describe current screen")  # VISION: Added command
        print("  'screenshot' - Save screenshot to file")  # VISION: Added command
        print("=" * 60)
        print()
        ...
        print(f"💡 TIP: Type 'gtts' to switch to online TTS")
        print()
        ...
        while True:
            try:
                ...
                if cmd == 'piper':
                    voice.set_tts_mode('piper')
                    voice.speak("Switched to Piper offline TTS")
                    continue
                
                if cmd == 'gtts':
                    voice.set_tts_mode('gtts')
                    voice.speak("Switched to G T T S online TTS")
                    continue
                
                if cmd == 'tts':
                    tts_mode = voice.get_tts_mode().upper()
                    mode_type = "offline" if tts_mode == "PIPER" else "online"
                    msg = f"Current TTS mode: {tts_mode} ({mode_type})"
                    print(f"\n🎙️ {msg}")
                    voice.speak(msg)
                    print()
                    continue
                ...
                # VISION: Handle 'see' command
                if cmd == 'see':
                    print("\n👁️  Analyzing screen...")
                    description = agent.see_screen(save_screenshot=True)
                    print("\n📊 Screen Description:")
                    print("=" * 60)
                    print(description)
                    print("=" * 60)
                    voice.speak("Screen captured and analyzed")
                    print()
                    continue
                
                # VISION: Handle 'screenshot' command
                if cmd == 'screenshot':
                    from datetime import datetime
                    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                    filename = f"screen_{timestamp}.png"
                    agent.screen_capture.save_screenshot(filename)
        ...
        print("  6. Voice packages (SpeechRecognition, whisper) installed")
        print("  7. TTS packages (gtts, pydub) installed for online TTS")
        return 1
