...
When you reference past conversations, be natural about it - you inherently remember, just like a human would."""
    ...
    def build_system_prompt_with_memories(self, user_message: str) -> str:
        ...
        try:
            ...
            # Second search: flip the query to find ANSWERS not just similar questions
            ...
            # Third search: PostgreSQL keyword search on key nouns
            # Strips question words to get searchable terms
            # e.g. "what are my kids names" -> search DB for "kids"
            ...
            for word in key_words[:3]:  # Search top 3 key words
                try:
                    ...
                    question_starters = ('what', 'who', 'when', 'where', 'why', 'how', 'is ', 'are ', 'do ', 'did ', 'can ', 'ok ', 'that')
                    for r in db_results:
    ...
    def parse_time_query(self, user_message: str) -> Optional[Dict[str, Any]]:
        ...
        # Pattern: "today" - only fire with explicit recall context, not casual use
    ...
    def chat(
        ...
        # Check for "forget/delete that" — deletes last assistant response from ChromaDB
        forget_phrases = [
            "that's wrong", "that is wrong", "forget that", "delete that",
            "that is incorrect", "that's incorrect", "remove that",
            "that was wrong", "that was incorrect", "wrong delete that",
            "thats wrong", "thats incorrect"
        ]
        msg_lower = user_message.lower().strip()
        if any(phrase in msg_lower for phrase in forget_phrases):
            try:
                ...
                if last_assistant:
                    ...
                    try:
                        ...
                        return "I tried to delete that but couldn't find it in memory. Can you tell me the correct information?"
            except Exception as e:
